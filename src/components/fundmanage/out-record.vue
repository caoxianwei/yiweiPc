            ...
            <table id="thetable" border="0" width="100%" cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    <tr v-for="item in data.list">
                        ...
                        <td>
                            <span v-if="item.applyFlag == 0">处理中</span>
                            <span v-if="item.applyFlag == 1">提款成功</span>
                            <span v-if="item.applyFlag == 2">已撤销</span>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    export default {
        ...
        methods :{
            ...
            async searchRecord(param){
                ...
                var data = await api.ajax('user/money/pageWithdrawList', this.form)
                if(data && data.code == '0'){
                    this.data = data.data
                } else {
                    errorMsg(data.msg)
