<template>
	<div class="changeContent">
		<el-form :model="form" label-width="100px">
			<el-form-item label="旧资金密码：">
				<el-input class="width200" v-model="form.oldkey"></el-input>
			</el-form-item>
			<el-form-item label="新资金密码：">
				<el-input class="width200" v-model="form.key"></el-input>
			</el-form-item>
			<el-form-item label="确认新密码：">
				<el-input class="width200" v-model="form.confirm"></el-input>
			</el-form-item>
		...
		methods :{
			...
			async onChange() {
				if (this.form.key != this.confirm ) {
					errorMsg('抱歉两次资金密码不一致！')
					return
				}
				var changeLoginPwd = await api.ajax('user/updateFundPwd', this.form)
		        if(changeLoginPwd && changeLoginPwd.code == '0'){
		          	succMsg(changeLoginPwd.msg)
		          	var that  = this;
					this.$nextTick(function(){
				  		that.$root.$emit("closeDialog-init")
					})
		        } else {
		          errorMsg(changeLoginPwd.msg)
		        }
			},
			cancel() {
				var that  = this;
