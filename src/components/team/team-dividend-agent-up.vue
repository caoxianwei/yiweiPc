        ...
        <div class="grey_box_top pdt5" style="clear:both">
            <div class="grey_box_bottom pdb10 pdt10">
                <ul class="form">
                    ...
                    <li>
                        <span class="formTit">生效日期:</span>
                        <span class="select ie9-select"><el-date-picker v-model="effectiveDate" :editable="false" :clearable="false" type="date" :picker-options="pickerOptions" class="myDate datePicker"></el-date-picker></span>
                    </li>
                    <li>
                        <span class="formTit">分红比例:</span>
                        <input type="text" v-model="range" class="input_txt" name="range" maxlength="6">%
                        <span class="range">(分红比例的范围在{{agentInfo.rate}}%到{{userInfo.rate - 1}}%之间)</span>
                    </li>
                    <li class="textareali">
...
<script lang="babel">
    ...
    import { errorMsg, succMsg, formatDate } from '../../tools/command'
    ...
    export default {
        ...
        mounted () {
            var now = new Date()
            this.effectiveDate = new Date(now.getTime() + 86400000)
        ...
        methods : {
            ...
            async upApply () {
                ...
                this.form.effectiveDate = formatDate(this.effectiveDate)
                const data = await api.ajax('team/dividendAgent/apply',this.form)
                if(data && data.code == '0'){
                    succMsg(data.msg)
                } else {
                    errorMsg(data.msg)
